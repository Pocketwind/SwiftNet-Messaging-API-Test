...
def SingleAck(accessToken, id, settings):
    ...
    response=requests.post(ackUrl, headers=headers, params=param, proxies=settings["proxies"], verify=True, timeout=5)
...
def MultiAck(accessToken, ids, settings):
    ...
    response=requests.patch(url,headers=headers,json=ackList,proxies=settings["proxies"],verify=True, timeout=5)
...
def MultiNak(accessToken, ids, reason, settings):
    ...
    url=settings["distUrl"]
    response=requests.patch(url,headers=headers,json=nackList,proxies=settings["proxies"],verify=True, timeout=5)
